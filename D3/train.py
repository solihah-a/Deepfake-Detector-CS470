...
if __name__ == '__main__':
    ...
    val_opt = get_val_opt()
    # Defaults for local Defactify export. Override with env var D3_DATA_ROOT if needed.
    data_root = os.environ.get("D3_DATA_ROOT", "../data/d3_defactify")

    # Evaluation roots should each contain real/ and fake/ subfolders.
    val_data_root = [
        os.path.join(data_root, "validation"),
        os.path.join(data_root, "test"),
    ]

    # Training roots for binary real/fake classification.
    real_folders = [os.path.join(data_root, "train", "real")]
    fake_folders = [os.path.join(data_root, "train", "fake")]

    if not all(os.path.exists(path) for path in real_folders + fake_folders):
        raise FileNotFoundError(
            "Training folders not found. Run prepare_d3_data.py first or set D3_DATA_ROOT."
        )

    for root in val_data_root:
        if not os.path.exists(os.path.join(root, "real")) or not os.path.exists(
            os.path.join(root, "fake")
        ):
            raise FileNotFoundError(
                f"Validation folder missing real/fake subfolders: {root}"
            )
