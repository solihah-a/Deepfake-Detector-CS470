...
if __name__ == '__main__':
    ...
    _parser.add_argument('--result_folder', type=str, default='../outputs/d3_pretrained_results')
    _parser.add_argument('--ckpt', type=str, default='./ckpt/classifier.pth')
    _parser.add_argument('--test_folders', type=str, nargs='+', default=['../data/d3_defactify/test'],
                         help='One or more folders each containing real/ and fake/ subfolders')
    _parser.add_argument('--batch_size', type=int, default=32)
    _parser.add_argument('--max_sample', type=int, default=None, help='Max images per class per folder. None = all')
    _parser.add_argument('--jpeg_quality', type=int, default=None)
    _parser.add_argument('--gaussian_sigma', type=int, default=None)
    _args = _parser.parse_args()

    # basic configuration
    set_seed(418)
    result_folder = _args.result_folder
    checkpoint = _args.ckpt
    test_folders = _args.test_folders
    batch_size = _args.batch_size
    jpeg_quality = _args.jpeg_quality
    gaussian_sigma = _args.gaussian_sigma
    exp_name = f"validation"

    max_sample = _args.max_sample
